...
| 항목 | 결정 | 대안과 이유 |
|---|---|---|
| 현재 폴더 선택 | 목록 맨 위에 `.` 항목. `DirsOnly`와 `Both`에서만 | 전용 키: 힌트 없이는 발견 불가, 커스텀 키맵 앱은 따로 바인딩 필요. `DirsOnly`에서 `Enter`를 선택으로 변경: 빈 폴더와 현재 폴더는 여전히 못 고르고 `Enter` 의미가 모드마다 달라짐. `.`은 셸 관례와 일치하고 새 키가 없으며 목록에 보이므로 힌트 자체가 됨 |
...
| 버전 | 0.4.0 | API는 안 깨지지만 `DirsOnly`/`Both`에서 `entries[0]`이 `.`이 되고 `Selected`가 `current_dir`를 돌려줄 수 있음. `entries`를 직접 순회하거나 인덱스로 접근하는 앱에는 관찰 가능한 변화라 patch로 내기엔 위험 |

## 1. `.` 항목

### 동작

- `refresh_entries`와 `FilePickerBuilder::build`에서 목록 읽기에 성공한 뒤, `mode != FilesOnly`이면 인덱스 0에 `Entry { name: ".", path: current_dir.clone(), kind: EntryKind::Directory, depth: 0 }`를 삽입한다. 읽기에 실패하면 삽입하지 않는다. 존재하지 않는 `start_dir`나 권한 없는 폴더를 "선택"해 앱에 돌려주는 일이 없어야 하고, 그 경우 화면은 0.3.0대로 `(empty)`와 `read_error`다.
...
- `PickerMode`의 rustdoc에 "`DirsOnly`와 `Both`에서는 목록 맨 위에 현재 폴더를 뜻하는 `.` 항목이 있다"를 적는다.
- `CommonState::entries`의 rustdoc에 같은 내용을 적어, `entries`를 직접 순회하는 앱이 알게 한다.
- README 키 바인딩 표와 Features에 한 줄.

### 테스트

- `DirsOnly`로 빌드하면 `entries[0].name == "."`이고 path가 `current_dir`다. `FilesOnly`에서는 없다.
...
- 숨김 토글을 해도 `.`은 남는다.
- 폴더에 진입하면 커서가 `.`에 있다.
- 읽기 실패 시에는 `.`이 없다. 기존 테스트 `unreadable_directory_reports_why_instead_of_looking_empty`와 `tree_view_reports_an_unreadable_root_too`가 기본 모드(`Both`)에서 `entries.is_empty()`를 단언하는데, 이 규칙 덕에 그대로 통과한다.
...
- `Cargo.toml` 0.4.0.
- CHANGELOG `[0.4.0] - unreleased`: Added(`.` 항목, 위치 표시, `dialogs` 예제), Changed(경로 바 잘림 방향), 그리고 "`entries`를 직접 순회하거나 `entries[0]`에 접근하는 앱은 `DirsOnly`/`Both`에서 `.`을 보게 된다"를 Migration 절에 명시.
